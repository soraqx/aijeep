...
type LiveMapViewProps = {
    ...
    calculateSpeed: (current: { gps: string; timestamp: number }, previous: { gps: string; timestamp: number }) => number;
    jeepneyIcon: L.Icon;
};
...
export function LiveMapView({
    ...
    // Build a map of jeepney positions (latest telemetry per vehicle) and track previous positions
    useEffect(() => {
        ...
        for (const [jeepneyId, telemetrys] of groupedByJeepney.entries()) {
            ...
            if (sorted.length >= 2) {
                ...
                previousPositions.set(jeepneyId, {
                    gps: previous.gps,
